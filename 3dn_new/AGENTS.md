...
5. Auth UI: adapt the scaffold's `pages/auth/Index.tsx` (`/auth` route, includes the
   email verification step) — never write a login page from scratch. Keep the awaited
   session refetch after the verify-code call: that endpoint is a product route, so
   the Better Auth client session does not refresh itself, and navigating without the
   refetch leaves every guard reading the pre-verification session.
